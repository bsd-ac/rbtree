...
static inline
struct rb_node *get_child(struct rb_node *node, unsigned short dir)
{
        assert(dir <= 2);
        return (struct rb_node *)((uintptr_t)node->child[dir] & ~RBT_LOW);
}

static inline
void set_child(struct rb_node *node, unsigned short dir, struct rb_node *child)
{
        assert(dir <= 2);
...
static inline
unsigned short get_child_rdiff(struct rb_node *node, unsigned short dir)
{
        assert(dir <= 2);
        return (unsigned short)((uintptr_t)node->child[dir] & RBT_LOW);
}

static inline
void set_child_rdiff(struct rb_node *node, short dir, unsigned short rdiff)
{
        assert(rdiff <= RBT_LOW);
        node->child[dir] = ((uintptr_t)get_child(node, dir)) & rdiff;
}

static inline void
rb_rotate(struct rb_node *parent, struct rb_node *elm, struct rb_node *par, int dir)
