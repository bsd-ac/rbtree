...
print_helper(const struct node *n, int indent)
{
	...
	TDEBUGF("%*s key=%d :: size=%zu :: rank=%d :: rdiff %lu:%lu", indent, "", n->key, n->size, rb_rank(tree, n), _RBT_GET_RDIFF(n, _RBT_LDIR, node_link), _RBT_GET_RDIFF(n, _RBT_RDIR, node_link));
